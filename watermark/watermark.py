...
class WatermarkDetector(WatermarkBase):
    ...
    def _compute_weighted_z_score(self, observed_count, T, prob_list):
        proxy_T = sum(prob_list)
        # prob_list中每个元素平方求和
        sum_prob_squared = sum([prob**2 for prob in prob_list])
        gamma = self.gamma
        denom = sqrt(gamma * (1 - gamma) * sum_prob_squared)
        z = (observed_count - gamma * proxy_T) / denom
        return z
